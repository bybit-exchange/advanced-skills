...
RATE_LIMIT_BACKOFF = [0.5, 1.0, 1.5]  # seconds per attempt (retCode 10006 or HTTP 429)
        ...
        for attempt in range(MAX_RATE_LIMIT_RETRIES + 1):
            ...
            try:
                ...
                if resp.status_code == 429 and attempt < MAX_RATE_LIMIT_RETRIES:
                    print(f"HTTP 429 rate limit on GET {endpoint}, retrying ({attempt + 1}/{MAX_RATE_LIMIT_RETRIES})...")
                ...
                data = resp.json()
                if data.get("retCode") == 10006 and attempt < MAX_RATE_LIMIT_RETRIES:
                    print(f"Rate limit (retCode 10006) on GET {endpoint}, retrying ({attempt + 1}/{MAX_RATE_LIMIT_RETRIES})...")
                    time.sleep(RATE_LIMIT_BACKOFF[attempt])
                    continue
                return data
    ...
    def post(self, endpoint, body=None):
        ...
        for attempt in range(MAX_RATE_LIMIT_RETRIES + 1):
            ...
            try:
                ...
                if resp.status_code == 429 and attempt < MAX_RATE_LIMIT_RETRIES:
                    print(f"HTTP 429 rate limit on POST {endpoint}, retrying ({attempt + 1}/{MAX_RATE_LIMIT_RETRIES})...")
                    time.sleep(RATE_LIMIT_BACKOFF[attempt])
                    continue
                if resp.status_code != 200 and not resp.text.strip():
                ...
                data = resp.json()
                if data.get("retCode") == 10006 and attempt < MAX_RATE_LIMIT_RETRIES:
                    print(f"Rate limit (retCode 10006) on POST {endpoint}, retrying ({attempt + 1}/{MAX_RATE_LIMIT_RETRIES})...")
                    time.sleep(RATE_LIMIT_BACKOFF[attempt])
                    continue
                return data
            except Exception as e:
